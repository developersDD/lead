...
SedemacApp.controller('logoManagementCTRL',['$scope', '$http', 'blockUI', '$controller', 'DTOptionsBuilder', '$mdDialog',
	function($scope, $http, blockUI, $controller, DTOptionsBuilder, $mdDialog){
		$controller('baseCTRL', { $scope: $scope })
		$controller('userCTRL', { $scope: $scope })
		
		var apiURL = appURL + 'admin/settings/logo/';
		...
		$scope.showStatusConfirm = function(ev, row) {
		    if(row.status_id == '1'){
		    	var message = 'Are you sure to make this logo inactive?';
		    }else{
		    	var message = 'Are you sure to make this logo active?';
		    }
		...
		/*------------------------------addUserCategory--------------------------*/
		$scope.newLogo = {};
		$scope.loadAddLogo = function(){
			window.location = appURL + 'admin/settings/logo/add';
		};
		...
		$scope.addLogo = function(isValid, form){
			if(isValid){
				...
				$http({  
		            ...
		            data: { model: $scope.newLogo, files: $scope.files }  
		        }).then(function(response){
					$scope.stopBlocker();
					var res = response.data;
					if(res.status == true){
		...
		$scope.updateLogo = function(isValid, form){
			if(isValid){
				if(editlogo.logo_image != "" || $scope.files.length > 0){
				$scope.fileReqVal = '';
				$scope.startBlocker();
				...
				$http({  
		            ...
		            data: { model: $scope.editlogo, files: $scope.files }  
		        }).then(function(response){
					$scope.stopBlocker();
					var res = response.data;
					if(res.status == true){
